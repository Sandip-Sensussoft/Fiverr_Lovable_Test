...
const generatePersonalizedContent = async (name: string, industry: string) => {
  try {
    ...
    return (
      ...
      `Hi ${name}! Welcome to our innovation community!`
    );
  } catch (error) {
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { name, email, industry, requestId } = requestData;

    console.log(
      `Processing email request for ${name} (${email}) from ${industry} industry. Request ID: ${
        requestId || "none"
      }`
    );
    ...
    console.log(`Generated content for ${name}`);
    ...
    const emailResponse = await resend.emails.send({
      from: "Innovation Community <testing-email@resend.dev>",
      to: [email],
    ...
    console.log(
      `Email sent successfully for ${name}. Request ID: ${requestId || "none"}`
    );
